...
import { AdditiveProps, MetricProps, NutrientProps } from "@/types";
...
export const verifyNutrient = (nutrient: NutrientProps): MetricProps | null => {
  // Check values
  if (nutrient.unitName === "" && nutrient.name === "") return null;
...
export const getBarUIDetails = (
  ...
  let arrowLeft = Math.round(
    (amount / metric.benchmarks_100g[metric.benchmarks_100g.length - 1]) * 100
  );
...
export const getBarPartsWidth = (metric: MetricProps): string[] => {
  ...
  benchmarks_100g.map((benchmark) => {
    let width = Math.round(
      ((benchmark - previousBenchmark) / largestBenchmark) * 100
    );
...
export const getAdditivesDetails = (additives: string[]): AdditiveProps[] => {
  ...
  additives.forEach((eNumber) => {
    let additiveInfo = additivesList.find(
      (additiveObj) => additiveObj.number === eNumber
    );
...
export const getAdditivesAmount = (
  additives: string[],
  metric: MetricProps
): number => {
...
export const rateProduct = (nutrients: NutrientProps[]): number => {
  ...
  nutrients.find((nutrient) => {
    if (nutrient.name === "additives") sum = 3 - nutrient.rate!;
  });
